...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.groupBy(x => x).map { case (k, ys) => (k, ys.length) }.toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    s.flatten.filter(p => p.isLetter).groupBy(x => x.toLower).map { case (k, ys) => (k, ys.length) }.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def concatOccurrences(occ: Occurrences): List[Char] = occ match {
      case Nil => Nil
      case (ch, f) :: xs => (for (i <- 1 to f) yield ch).toList ++ concatOccurrences(xs)
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    (for {
      (k, v) <- xMap
      newCount = v - yMap.getOrElse(k, 0)
      if newCount > 0
    } yield (k, newCount)).toList
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsByOcc(occ: Occurrences): List[Sentence] = occ match {
      case Nil => List(List())
      case List() => List(List())
      case _ =>
